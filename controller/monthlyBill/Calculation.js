...
/**
 * Adding fixed charge to the fixed bill accordingly controller
 * @param {*} noOfUnits
 * @param {*} unitDetails
 * @returns
 */
function caculateFixedBill(noOfUnits, unitDetails) {
  ...
  if (noOfUnits <= 30) {
    billValue =
      ...
      parseFloat(unitDetails[0].Fixed_charge);
    ...
    billValue =
      parseFloat((noOfUnits - 60) * unitDetails[4].Unit_charge) +
      parseFloat(60 * unitDetails[1].Unit_charge) +
      parseFloat(unitDetails[4].Fixed_charge);
    return billValue.toFixed(2);
  } else if (noOfUnits <= 120) {
    billValue =
      parseFloat((noOfUnits - 90) * unitDetails[5].Unit_charge) +
      parseFloat(60 * unitDetails[1].Unit_charge) +
      parseFloat(30 * unitDetails[4].Unit_charge) +
      parseFloat(unitDetails[5].Fixed_charge);
    return billValue.toFixed(2);
  } else if (noOfUnits <= 180) {
    billValue =
      parseFloat((noOfUnits - 120) * unitDetails[2].Unit_charge) +
      parseFloat(60 * unitDetails[1].Unit_charge) +
      parseFloat(30 * unitDetails[4].Unit_charge) +
      parseFloat(30 * unitDetails[5].Unit_charge) +
      parseFloat(unitDetails[2].Fixed_charge);
    return billValue.toFixed(2);
  } else if (noOfUnits > 180) {
    billValue =
      parseFloat((noOfUnits - 180) * unitDetails[6].Unit_charge) +
      parseFloat(60 * unitDetails[1].Unit_charge) +
      parseFloat(30 * unitDetails[4].Unit_charge) +
      parseFloat(30 * unitDetails[5].Unit_charge) +
      parseFloat(60 * unitDetails[2].Unit_charge) +
      parseFloat(unitDetails[6].Fixed_charge);
    return billValue.toFixed(2);
  }
}

/**
 * Adding fixed charge to the ToU bill controller
 * @param {*} TOU_bill_sum
 * @param {*} fixed_charge
 * @returns
 */
...
/**
 * Get calculated bill values from each devices controller
 * @param {*} request
 * @param {*} response
 */
...
async function getcalculatedBillValue(request, response) {
  try {
    ...
    var Bill_details = await calculateModel.getDeviceDetailsToCalculate(
      billId,
      CustId
    );
...
/**
 * Save the calculated bill values controller
 * @param {*} request
 * @param {*} response
 */
async function calculatedBillValue(request, response) {
  try {
    ...
    if (
      Bill_details.data[0].fixed_bill_cost > Bill_details.data[0].TOU_bill_cost
    ) {
    ...
    await calculateModel.setMonthlyPlan(
      Bill_details.data[0],
      CustId,
      best_model
    );
...
/**
 * Update the monthly bill plans controller
 * @param {*} request
 * @param {*} response
 */
...
async function calculatedBillValueUpdate(request, response) {
  try {
    ...
    // console.log(Bill_details);
    await calculateModel.updateMonthlyPlan(
      Bill_details.data[0],
      CustId,
      best_model
    );
...
/**
 * Update the calculated monthly bill plan after applying suggestions controller
 * @param {*} billId
 * @param {*} CustId
 */
async function calculatedBillValueUpdateAfterApplySuggest(billId, CustId) {
  try {
    var best_model;
...
module.exports = {
  calculatedBillValue,
  getcalculatedBillValue,
  calculatedBillValueUpdate,
  calculatedBillValueUpdateAfterApplySuggest,
};
