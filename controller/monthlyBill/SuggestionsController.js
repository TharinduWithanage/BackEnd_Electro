...
async function ApplySuggestions(request, response) { 
    ...
    try {
        ...
        var DeviceData = await addDeviceModel.getDeviceDetailsMailBill(suggestDetails.bill_id, CustId, suggestDetails.device_id);
        ...
        if(suggestDetails.cur_time == 'peak'){
            ...
            if(suggestDetails.change_time == 'off peak'){
                DeviceDetails.hPeak = DeviceDetails.hPeak - suggestDetails.can_change_hours;
                DeviceDetails.mPeak = DeviceDetails.mPeak - suggestDetails.can_change_minutes;
                DeviceDetails.hoffPeak = DeviceDetails.hoffPeak + suggestDetails.can_change_hours;
                DeviceDetails.mOffPeak = DeviceDetails.mOffPeak + suggestDetails.can_change_minutes;

            }else if(suggestDetails.change_time == 'day'){
                DeviceDetails.hPeak = DeviceDetails.hPeak - suggestDetails.can_change_hours;
                DeviceDetails.mPeak = DeviceDetails.mPeak - suggestDetails.can_change_minutes;
                DeviceDetails.hDay = DeviceDetails.hDay + suggestDetails.can_change_hours;
                DeviceDetails.mDay = DeviceDetails.mDay + suggestDetails.can_change_minutes;
        ...
        await monthlyBillCalculate.updateDeviceWithApplySugestion(DeviceDetails, CustId );

        await calculation.calculatedBillValueUpdateAfterApplySuggest(suggestDetails.bill_id ,CustId);
        
        var Suggestions = await SuggestionsModel.getSuggestions(suggestDetails.bill_id , CustId);
